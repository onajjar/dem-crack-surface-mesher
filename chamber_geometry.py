...
def _replace_required_assignment(block: str, name: str, expression: str) -> str:
    ...
    if count != 1:
        raise ValueError(
            "The single mesh DGIBI must contain exactly one Main Program "
...
def patch_chamber_program(program: str, chambers: ChamberParameters) -> str:
    """Set the native Cast3M chamber toggle and its scalar parameters."""

    chambers.validated()
    marker = program.find(MAIN_BLOCK_START)
    if marker < 0:
        raise ValueError("Could not find the Main Program block in the mesh DGIBI.")
    head = program[:marker]
    block = program[marker:]
    assignments = (
        ("opti_chamb", "1" if chambers.enabled else "0"),
